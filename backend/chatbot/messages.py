...
def send_welcome_menu(to: str, member_name: str = None) -> dict:
    """Send welcome/main menu."""
    if member_name:
        greeting = f"Hello {member_name}!"
    else:
        greeting = "Hello!"
    ...
    return send_buttons(
        ...
        buttons=buttons,
        header="LifeTap",
        footer="Emergency response at your fingertips"
    )


def send_not_registered(to: str) -> dict:
    """Send message for unregistered users."""
    buttons = [
        {'id': 'register_start', 'title': 'Register Now'},
        {'id': 'menu_help', 'title': 'Learn More'}
    ]

    return send_buttons(
        to=to,
        body="You're not registered with LifeTap yet.\n\nJoin today to get emergency medical response coverage starting from just $1/month.",
        buttons=buttons,
        header="Join LifeTap",
        footer="Protect yourself and your loved ones"
    )


def send_subscription_expired(to: str, member_name: str) -> dict:
    """Send subscription expired message."""
    buttons = [
        {'id': 'renew_subscription', 'title': 'Renew Now'},
        {'id': 'menu_help', 'title': 'Contact Support'}
    ]

    return send_buttons(
        to=to,
        body=f"Hi {member_name},\n\nYour LifeTap subscription has expired. Renew now to continue your emergency coverage.",
        buttons=buttons,
